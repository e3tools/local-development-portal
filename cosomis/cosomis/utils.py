...
def clear_investment_description():
    investments = []
    for inv in Investment.objects.filter(description__isnell=False):
        inv.description = inv.description if inv.description and inv.description.strip() not in IGNORES else None
