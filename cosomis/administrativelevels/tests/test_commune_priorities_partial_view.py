"""
Integration tests for CommunePrioritiesPartialView.
"""
...
def _make_commune(name="Commune Test", **kwargs):
...
def _make_canton(name, parent, **kwargs):
    return AdministrativeLevel.objects.create(
        name=name,
...
class CommunePrioritiesPartialViewTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.user = _make_user()
        cls.commune = _make_commune(name="Commune A")
        cls.canton = _make_canton(name="Canton A", parent=cls.commune)
        ...
        cls.url = reverse("administrativelevels:commune_priorities_partial", args=[cls.commune.pk])
    ...
    def test_template_used(self):
        response = self.client.get(self.url)
        self.assertTemplateUsed(response, "commune/tabs/priorities.html")

    def test_context_data(self):
        response = self.client.get(self.url)
        self.assertIn('investments', response.context)
        priorities = response.context['investments']
        self.assertTrue(any(p.id == self.investment.id for p in priorities))

    def test_filters_applied(self):
        # Create another investment with different status
        inv2 = _make_investment(self.village, self.sector, status=Investment.FUNDED)
        
        # Test filtering by status
        response = self.client.get(self.url + f"?funding-status-filter={Investment.NOT_FUNDED}")
        priorities = response.context['investments']
        self.assertTrue(all(p.project_status == Investment.NOT_FUNDED for p in priorities))
        self.assertTrue(any(p.id == self.investment.id for p in priorities))
        self.assertFalse(any(p.id == inv2.id for p in priorities))
